...
void n_vtiFile_4D_int_write_f(int *x, int *y, int *z, int *c, int **data, const char *fileName)
{
    n_vtiFile_4D_int_write(*x, *y, *z, *c, NPTR2ARR4D(int,*data,*x, *y, *z, *c), fileName);
}

void n_vtiFile_4D_int_write(int x, int y, int z, int c, int data[x][y][z][c], const char *fileName)
{
    ...
    for (i = 0; i < c; i++)
    {
        ...
        rc = xmlTextWriterWriteAttribute(writer, BAD_CAST "type", BAD_CAST "Int32");
        sprintf(temp1, "%i", i);
    ...
    for (l = 0; l < c; l++)
    {
        ...
        for (k = 0; k < z; k++)
        {
            for (j = 0; j < y; j++)
            {
                for (i = 0; i < x; i++)
                {
                    sprintf(temp, MY_VTI_FORMAT_INT, data[i][j][k][l]);
                    fputs(temp, fp);
                }
            }
        }
...
void n_vtiFile_4D_int_read_f(int *x, int *y, int *z, int *c, int **data, const char *fileName)
{
    n_vtiFile_4D_int_read(*x, *y, *z, *c, NPTR2ARR4D(int,*data,*x, *y, *z, *c), fileName);
}

void n_vtiFile_4D_int_read(int x, int y, int z, int c, int data[x][y][z][c], const char *fileName)
{
                ...
                for (k = 0; k < z; k++)
                {
                    for (j = 0; j < y; j++)
                    {
                        for (i = 0; i < x; i++)
                        {
                            ...
                            // }
                            fscanf(fp, "%i", &data[i][j][k][dataCount]);
