...
void n_vtiFile_4D_short_write_f(int *x, int *y, int *z, int *c, short **data, const char *fileName)
{
    n_vtiFile_4D_short_write(*x, *y, *z, *c, NPTR2ARR4D(short,*data,*x, *y, *z, *c), fileName);
}

void n_vtiFile_4D_short_write(int x, int y, int z, int c, short data[x][y][z][c], const char *fileName)
{
    ...
    for (i = 0; i < c; i++)
    {
        ...
        rc = xmlTextWriterWriteAttribute(writer, BAD_CAST "type", BAD_CAST "UInt16");
        sprintf(temp1, "%i", i);
    ...
    for (l = 0; l < c; l++)
    {
        ...
        for (k = 0; k < z; k++)
        {
            for (j = 0; j < y; j++)
            {
                for (i = 0; i < x; i++)
                {
                    sprintf(temp, MY_VTI_FORMAT_short, data[i][j][k][l]);
                    fputs(temp, fp);
                }
            }
        }
...
void n_vtiFile_4D_short_read_f(int *x, int *y, int *z, int *c, short **data, const char *fileName)
{
    n_vtiFile_4D_short_read(*x, *y, *z, *c, NPTR2ARR4D(short, *data,*x, *y, *z,*c), fileName);
}

void n_vtiFile_4D_short_read(int x, int y, int z, int c, short data[x][y][z][c], const char *fileName)
{
    ...
    while (!endReached)
    {
        ...
        if (pos != -1)
        {
            ...
            // ZF_LOGD("The data array with shift %li %i %i", ftell(fp),pos);
            ...
            {
                ignoreNext = 1;
                ZF_LOGD("Find dataarray is not ignored %i %i",ignoreNext,dataCount);
                ...
                for (k = 0; k < z; k++)
                {
                    for (j = 0; j < y; j++)
                    {
                        ...
                        {
                            // fscanf(fp, "%i", intTemp);
                            // ZF_LOGD("int temp %i",intTemp);
                            // data[i][j][k][dataCount]=intTemp;
                            // if (data[i][j][k][dataCount]!=0)
                            // {
                            //     ZF_LOGD("none zero found",i,j,k,data[i][j][k][dataCount]);
                            // }
                            fscanf(fp, "%hu", &data[i][j][k][dataCount]);
